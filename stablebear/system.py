...
def force_cpu(on: bool):
    """Set forced execution on CPU. By default, execution may happen on either CPU or GPU (if using a GPU-enabled build of stablebear).

    Parameters
    ----------
    on : bool
      If `True`, force execution on CPU for all operations. If `False`, execution may happen on either CPU or GPU (if using a GPU-enabled build of stablebear).
    """
    cpp.force_cpu(on)
...
def limit_gpus(n: int):
    """Sets the number of GPUs that can be used by stablebear. By default, all available GPUs are used.

    This option only has an effect if stablebear is compiled with GPU support.
...
def build_type() -> str:
    """Return the build type of the stablebear backend.
