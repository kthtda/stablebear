...
def _add_delvewheel_libs_dir():
    """Add the sibling masspcf.libs directory to the DLL search path on Windows.

    delvewheel mangles bundled DLL filenames (e.g. msvcp140.dll ->
    msvcp140-<hash>.dll) and patches masspcf/__init__.py to call
    os.add_dll_directory at package import time. Callers that load
    _mpcf_cuda*.pyd directly via importlib (bypassing the package init)
    must replicate that step or the loader cannot resolve the mangled CRT.
    """
    if sys.platform != "win32":
        return
    libs_dir = pathlib.Path(__file__).resolve().parent.parent / "masspcf.libs"
    if libs_dir.is_dir():
