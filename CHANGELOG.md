...
* **Persistence surfaces input errors instead of returning empty barcodes** — `compute_persistent_homology` on a malformed point cloud (any per-cloud rank ≠ 2, e.g. a 1-D or 3-D array) silently returned all-empty barcodes: the C++ code deliberately threw a `runtime_error`, but that exception — raised inside the parallel walk — was stored in the task's future and never retrieved, because the wait path only synchronized (it never called `.get()`). Tasks now rethrow a worker's stored exception once the task completes, so the malformed-input error propagates to Python as a `RuntimeError` ("Point cloud at index … has unexpected shape …") instead of being swallowed. ([#27](https://github.com/kthtda/stablebear/issues/27))
* **Reductions accept a negative `dim` (NumPy axis semantics)** — `mean(A, dim=-1)` and `max_time(A, dim=-1)` raised a misleading pybind `TypeError` ("incompatible function arguments") because the C++ binding's `dim` is an unsigned `size_t`, so a negative axis never reached the library. `dim` is now normalized in the Python wrapper: a negative value counts from the last axis (`dim=-1` is the last axis, `dim=-2` the second-to-last), matching NumPy and the rest of the tensor API (`t[-1]`, `stack(axis=-1)`). An out-of-range `dim` now raises `IndexError`. ([#26](https://github.com/kthtda/stablebear/issues/26))
...
* **`np.asarray` on a `DistanceMatrix` or `SymmetricMatrix` now returns the dense matrix** — `np.asarray(D)` / `np.array(D)` on a `pdist` result (`DistanceMatrix`) or a `SymmetricMatrix` silently returned a 0-d object array wrapping the C++ object rather than the dense `(n, n)` matrix, a quiet correctness trap. Both classes now implement `__array__` (delegating to `to_dense()`), so `np.asarray`/`np.array` yield the dense matrix — consistent with `Pcf` and `FloatTensor` — plus a `to_numpy()` alias. ([#75](https://github.com/kthtda/stablebear/issues/75))
