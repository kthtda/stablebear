...
* **`bool()` on a numeric tensor reflects the value, not the length** — `FloatTensor`/`IntTensor` had no `__bool__`, so `bool()` fell back to `__len__` and returned the first-dimension length: every non-empty numeric tensor was truthy and a 0-d tensor raised `len() of unsized object`. A single-element tensor (including a 0-d scalar) now returns its element's truthiness and any other size raises `ValueError`, matching NumPy. ([#45](https://github.com/kthtda/stablebear/issues/45))
