...
* **`squeeze` and `expand_dims` accept a tuple of axes, and `squeeze` is a no-op on a 0-d tensor** — passing a tuple of axes raised a raw `TypeError`, and `squeeze(0)` on a 0-d tensor raised `IndexError`. Both now accept a tuple/list of axes (rejecting duplicates and non-size-1 squeeze axes with `ValueError`), and squeezing axis 0/-1 of a 0-d tensor returns it unchanged, matching NumPy. ([#70](https://github.com/kthtda/stablebear/issues/70), [#71](https://github.com/kthtda/stablebear/issues/71))
