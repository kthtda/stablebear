...
* **`SymmetricMatrix` / `DistanceMatrix` support `shape`, `len`, `diagonal()`, and row/slice indexing** — a compressed matrix previously exposed only `(i, j)` scalar access. It now has a `shape` property, `len()`, a `diagonal()` accessor, and `m[i]` / `m[i, :]` / `m[:, j]` / `m[a:b, c:d]` indexing (returning NumPy arrays), while `m[i, j]` still returns the scalar entry. ([#79](https://github.com/kthtda/stablebear/issues/79))
