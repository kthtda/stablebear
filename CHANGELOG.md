...
* **`bool()` on a numeric tensor reflects the value, not the length** — a single-element tensor (including a 0-d scalar) now returns its element's truthiness and any other size raises `ValueError`, matching NumPy; previously `bool()` fell back to `__len__` and reported the first-dimension length. ([#45](https://github.com/kthtda/stablebear/issues/45))
