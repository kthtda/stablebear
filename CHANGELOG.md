...
* **A size-1 tensor right-hand side broadcasts on masked/index assignment** — `a[mask] = sb.FloatTensor([v])` (and integer-index / outer assignment with a size-1 tensor RHS) raised a count-mismatch error instead of broadcasting the single value across the selection. A size-1 tensor RHS is now broadcast like a scalar (matching NumPy); multi-element shape mismatches still raise. ([#60](https://github.com/kthtda/stablebear/issues/60))
