...
* **Non-numeric tensors export back to NumPy** — `PointCloudTensor` (of equal-sized clouds) and `DistanceMatrixTensor`/`SymmetricMatrixTensor` now provide `to_dense()`/`to_numpy()` (and work with `np.asarray`), and `BarcodeTensor` provides `to_numpy()`/`tolist()` returning the per-cell barcode arrays — instead of only the per-element loop. Ragged point clouds / non-uniform matrices raise a clear `ValueError`. ([#85](https://github.com/kthtda/stablebear/issues/85), [#93](https://github.com/kthtda/stablebear/issues/93), [#94](https://github.com/kthtda/stablebear/issues/94))
