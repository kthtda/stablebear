...
* **Tensor construction reports bad inputs clearly** — `FloatTensor` no longer silently ignores a non-float `dtype=` (it raises `TypeError`), `IntTensor` with an invalid `dtype=` raises `TypeError` instead of a bare `KeyError`, building a PCF/barcode tensor from a list with a non-element or mixed-precision item raises a clear `TypeError` (instead of an `AttributeError` or raw pybind dump — this also covers `l2_kernel` of a mixed-dtype list), and `BarcodeTensor` can be built directly from a list of raw `(n, 2)` NumPy barcode arrays. ([#54](https://github.com/kthtda/stablebear/issues/54), [#55](https://github.com/kthtda/stablebear/issues/55), [#80](https://github.com/kthtda/stablebear/issues/80), [#83](https://github.com/kthtda/stablebear/issues/83))
