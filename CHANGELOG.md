...
* **System setters validate their arguments** — negative or zero arguments to `system.limit_cpus`/`limit_gpus`/`set_cuda_threshold`/`set_parallel_eval_threshold`/`set_min_block_side`/`set_block_size` raised a leaky pybind overload dump or an internal taskflow `RuntimeError`. They now raise a clear `ValueError` naming the parameter and its valid bound. ([#101](https://github.com/kthtda/stablebear/issues/101))
