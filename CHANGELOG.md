...
* **Mismatched-dtype tensor arithmetic raises a clear error** — adding, subtracting, multiplying, or dividing two tensors of different dtype (`pcf32 + pcf64`, `pcf32 + pcf32i`, `float32 + float64`, and the in-place variants) leaked the raw pybind "incompatible function arguments" dump. It now raises a clear `TypeError` naming both dtypes (there is no implicit promotion yet), matching the clean `Pcf + Pcf` mismatch error. Same-dtype arithmetic and scalar / `Pcf` / array right-hand sides are unchanged. ([#64](https://github.com/kthtda/stablebear/issues/64))
