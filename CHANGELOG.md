...
* **`plot_barcode` now rejects invalid inputs clearly** — unsupported inputs and array dtypes raise `TypeError`; arrays not shaped `(n, 2)` and barcode tensors with more than one dimension raise `ValueError`. Validation occurs before a plotting axes is created, so invalid calls no longer leave an empty figure behind. ([#159](https://github.com/kthtda/stablebear/issues/159))
...
* **Assigning a scalar to a `PointCloudTensor` cell raises a clear error** — a point-cloud cell holds an `(n_points, dim)` array, but `t[i] = 3.14` passed setitem validation and then leaked the internal `Cannot create FloatTensor from <class 'float'>` error. Scalars are no longer advertised as valid right-hand sides for a point-cloud cell, so the assignment now raises a clear `TypeError`; assigning an array (or `FloatTensor`) is unchanged. ([#41](https://github.com/kthtda/stablebear/issues/41))
* **Mismatched-precision matrix assignment raises a clear error** — assigning a `float32` `DistanceMatrix`/`SymmetricMatrix` into a 64-bit matrix tensor (or a `distmat32`/`symmat32` tensor into a 64-bit one, and vice versa) leaked the raw pybind `_set_element` / `__setitem__` overload dump. The 32/64-bit precision is now checked up front, raising a `TypeError` that names both dtypes; matching-precision assignment is unchanged. ([#42](https://github.com/kthtda/stablebear/issues/42))
