...
* **`DistanceMatrix` rejects NaN entries** — the nonnegativity check used `value < 0`, which is false for NaN, so a NaN distance slipped past the documented "nonnegative" contract (and via `from_dense` surfaced as a misleading "Matrix must be symmetric" error, since `NaN != NaN`). The guard now rejects NaN as well as negatives on element assignment, the `(n, init)` constructor, and `from_dense`, while still accepting `+inf` (a meaningful infinite distance). ([#43](https://github.com/kthtda/stablebear/issues/43))
