...
* **`concatenate` / `split` / `array_split` accept a negative axis and negative split indices, and report dtype mismatches clearly** — these took an unsigned axis, so a negative axis raised a raw pybind "incompatible function arguments" error; a negative split-index entry leaked the same error; and joining tensors of different dtype/precision leaked the pybind overload dump. A negative `axis` now resolves from the end (as in NumPy), negative split indices offset from the end of the axis (clamped at 0, matching `numpy.split`), and `concatenate`/`stack` raise a clear `TypeError` when the inputs do not share a dtype (there is no implicit upcasting — cast with `astype` first). `stack` now resolves its (new) axis Python-side too, so an out-of-range axis raises a clean `IndexError` consistent with `concatenate`/`split` rather than a C++ `ValueError`. ([#20](https://github.com/kthtda/stablebear/issues/20), [#21](https://github.com/kthtda/stablebear/issues/21), [#73](https://github.com/kthtda/stablebear/issues/73), [#74](https://github.com/kthtda/stablebear/issues/74))
