...
* **Assigning an array into a `PointCloudTensor` slice distributes one cloud per cell** — `t[:] = arr` (and `t[...]` / partial-index `t[i] = arr`) on a `PointCloudTensor` previously stored the entire array as one cloud broadcast into every selected cell. An ndarray right-hand side is now distributed across the selected cells (leading axes index cells, trailing `(n_points, dim)` axes form each cloud, like `PointCloudTensor(arr)`); a shape that does not match the selected region raises `ValueError`. ([#40](https://github.com/kthtda/stablebear/issues/40))
