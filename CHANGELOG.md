...
* **`BoolTensor` has a readable `repr`/`str`** — a `BoolTensor` (e.g. the result of `t > 3`) printed as `<...BoolTensor object at 0x...>` in the REPL and notebooks. It now renders its underlying boolean array like the numeric tensors. ([#96](https://github.com/kthtda/stablebear/issues/96))
