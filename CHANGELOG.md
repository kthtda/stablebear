...
* **`Pcf` gains `repr`, `dtype`, array accessors, and `from_arrays`** — a `Pcf` now has a readable `repr`, a `.dtype` property (`pcf32`/`pcf64`/`pcf32i`/`pcf64i`), `.times` / `.values` / `.breakpoints` arrays and `.t_min` / `.t_max` domain bounds (where `t_max` is the last breakpoint time, not infinity), and a `Pcf.from_arrays(times, values, dtype=None)` constructor that stacks separate time and value arrays. ([#66](https://github.com/kthtda/stablebear/issues/66), [#67](https://github.com/kthtda/stablebear/issues/67), [#68](https://github.com/kthtda/stablebear/issues/68), [#69](https://github.com/kthtda/stablebear/issues/69))
