...
* **`iterate_rectangles` works on integer PCFs with default bounds** — calling `iterate_rectangles(f, g)` on integer-typed PCFs raised a raw pybind `TypeError` because the default bounds (`a=0.0`, `b=inf`) could not bind to the integer-typed C++ overload. The defaults are now dtype-aware: an integer PCF uses `0` and the dtype's maximum as the unbounded right edge (an explicit `inf` upper bound is mapped to the maximum, any other bound is truncated to `int`); float PCFs are unchanged. ([#13](https://github.com/kthtda/stablebear/issues/13))
