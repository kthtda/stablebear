...
* **`Pcf` / tensor `** ` propagate the `RuntimeWarning` under `-W error`** — the power operators emitted a `RuntimeWarning` for a NaN/inf result via `PyErr_WarnEx` but ignored its return code, so when warnings were escalated to errors (`python -W error` or `warnings.simplefilter("error")`) the pending Python exception was left unhandled and pybind11 raised a confusing `SystemError` instead. The warning is now re-raised as the expected `RuntimeWarning`; default behavior (emit and continue) is unchanged. ([#12](https://github.com/kthtda/stablebear/issues/12))
