...
* **Cross-dtype numeric tensor comparisons work** — comparing two numeric tensors of different dtype (`float == int`, `int32 == int64`, the ordering operators, and `array_equal`) leaked the raw pybind overload error because the C++ bindings only accept a same-typed right-hand side. They now promote via NumPy and compare by value (so `IntTensor([1]) == FloatTensor([1.5])` is `False`, not silently truncated), returning a `BoolTensor`; same-dtype comparisons keep the C++ fast path. ([#49](https://github.com/kthtda/stablebear/issues/49))
