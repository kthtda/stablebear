...
* **Out-of-range reduction `dim` raises `numpy.AxisError`** — `mean`/`max_time` raised a plain `IndexError` for an out-of-range `dim`; they now raise `numpy.AxisError` (which subclasses `IndexError`, so existing handlers keep working), matching NumPy's axis-error contract. ([#82](https://github.com/kthtda/stablebear/issues/82))
