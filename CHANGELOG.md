...
* **`load` reports the real error on a corrupt tensor file** — loading a truncated/corrupt single-tensor file surfaced a misleading "Expected format type SingleObject ... but got format type SingleTensor" message, because `load` blindly retried the single-object path after any tensor-load error. It now only falls back to the object path for a genuine format-type mismatch and otherwise re-raises the real corruption/EOF error. ([#38](https://github.com/kthtda/stablebear/issues/38))
