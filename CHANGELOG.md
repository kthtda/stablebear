...
* **A fresh point-cloud tensor cell reads back as an empty cloud** — a never-assigned cell of `zeros(dtype=pcloud32/pcloud64)` read back as a 0-d scalar `0.0` (the C++ default-constructed value) instead of the documented "empty point cloud". It now reads back as an empty `(0, 2)` cloud, consistent with the fresh-cell behavior of the matrix and barcode dtypes. ([#44](https://github.com/kthtda/stablebear/issues/44))
