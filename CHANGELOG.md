...
* **Float scalar/slice assignment into an `IntTensor` truncates instead of erroring** — `t[i] = 7.9` (and `t[a:b] = 7.9`) on an integer tensor raised a raw pybind `TypeError`. The float right-hand side is now truncated toward zero (`7`, and `-7` for `-7.9`), matching NumPy's integer assignment. ([#7](https://github.com/kthtda/stablebear/issues/7))
* **Fractional and negative exponents on an `IntTensor` follow NumPy** — `int_tensor ** 0.5` raised a raw pybind error and `int_tensor ** -1` silently returned `0`. A fractional (or whole-float) exponent now promotes to a `float64` `FloatTensor`, and a negative integer exponent raises `ValueError` ("Integers to negative integer powers are not allowed."), both matching NumPy; a non-negative integer exponent keeps the integer result. ([#65](https://github.com/kthtda/stablebear/issues/65))
