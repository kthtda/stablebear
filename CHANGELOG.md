...
* **`allclose` handles infinities, shape mismatches, and more types** — three fixes: (1) it broke reflexivity for infinities (`|inf - inf|` is `NaN`, so `allclose(x, x)` was `False` when `x` contained `inf`, and opposite-sign infinities compared close); same-sign infinities are now close and finite-vs-inf / opposite-sign pairs are not, matching NumPy ([#51](https://github.com/kthtda/stablebear/issues/51)). (2) A shape mismatch silently returned `False`; mismatched shapes are now broadcast (as the elementwise operators do) or raise `ValueError` when genuinely incompatible ([#52](https://github.com/kthtda/stablebear/issues/52)). (3) `allclose` now also accepts `IntTensor`, `BoolTensor`, and NumPy arrays (compared as `float64`, dtypes may differ), not only `FloatTensor`/`DistanceMatrix`/`SymmetricMatrix` ([#100](https://github.com/kthtda/stablebear/issues/100)).
