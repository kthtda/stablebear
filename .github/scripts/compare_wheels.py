...
IGNORE_GLOBS = [
    ...
    "masspcf.libs/.load-order-*",  # delvewheel DLL load-order file (Windows only)
    "*.dist-info/DELVEWHEEL",  # delvewheel metadata stamp (Windows only)
]
...
def read_wheel(whl):
    ...
    filtered = {f for f in all_files if not should_ignore(f) and not f.endswith("/")}
    return filtered, raw
        ...
        for wheel_name in wheel_names:
            with zf.open(wheel_name) as wf:
                ...
                with zipfile.ZipFile(wheel_data) as whl:
                    wheels[Path(wheel_name).name], raw[Path(wheel_name).name] = (
                        read_wheel(whl)
                    )
    return wheels, raw
